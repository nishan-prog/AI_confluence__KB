...
// ---- Jira API setup ----
const JIRA_BASE_URL = process.env.JIRA_SITE.replace(/\/$/, '');
...
async function pollJiraTickets() {
  try {
    const lastPollTimestamp = state.lastPollTimestamp || 0;
    const lastPollDate = new Date(lastPollTimestamp).toISOString().split("T")[0]; // YYYY-MM-DD

    const res = await axios.get(`${JIRA_BASE_URL}/rest/api/3/search`, {
      auth: { username: JIRA_USER_EMAIL, password: JIRA_API_TOKEN },
      params: {
        jql: `status = Resolved AND resolutiondate >= "${lastPollDate}" ORDER BY resolved DESC`,
        fields: "summary,description,assignee,resolutiondate",
        maxResults: 50,
      },
    });
    ...
    for (const issue of issues) {
      ...
      const summaryText = issue.fields.summary || "No summary";
      const descriptionText = issue.fields.description || "No description";
      const assigneeEmail = issue.fields.assignee?.emailAddress;

      const resolvedTimestamp = issue.fields.resolutiondate ? new Date(issue.fields.resolutiondate).getTime() : 0;

      // Skip if ticket already processed
      if (state.processedTickets?.includes(ticketKey)) continue;

      if (assigneeEmail && resolvedTimestamp > lastPollTimestamp) {
        const summary = `Gemini Summary: ${summaryText}\n\n${descriptionText}`;
        reviewQueue.push({ subject: `${ticketKey} - ${summaryText}`, body: summary });

        console.log(`📝 Ticket added to review queue: ${ticketKey} - ${summaryText}`);
        await sendReviewEmail(assigneeEmail, `${ticketKey} - ${summaryText}`, summary);

        // Track processed tickets
        state.processedTickets = state.processedTickets || [];
        state.processedTickets.push(ticketKey);
      }
    }
...
async function sendReviewEmail(assigneeEmail, subject, summary) {
  try {
    const rawMessage = [
      ...
      "",
      `<p>Hi,</p>
      <p>A new internal ticket summary generated by Gemini is ready for review:</p>
...
async function postToConfluence() {
  if (reviewQueue.length === 0) {
    console.log("📭 No tickets in review queue.");
    return;
  }
